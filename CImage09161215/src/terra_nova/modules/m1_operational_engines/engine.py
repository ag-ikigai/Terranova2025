...
# ---- create_* aliases ----------------------------------------------------------
def create_revenue_schedule(*args, **kwargs):       return build_revenue_schedule(*args, **kwargs)
def create_opex_schedule(*args, **kwargs):          return build_opex_schedule(*args, **kwargs)
def create_capex_schedule(*args, **kwargs):         return build_capex_schedule(*args, **kwargs)
def create_depreciation_schedule(*args, **kwargs):  return build_depreciation_schedule(*args, **kwargs)

# ---- specific legacy convenience names ----------------------------------------
def calculate_steady_state_revenue(*args, **kwargs):
    # Older callers used this name for the revenue monthly distribution
    return build_revenue_schedule(*args, **kwargs)

def distribute_revenue_monthly(*args, **kwargs):
    # Legacy helper -> new unified builder
    return build_revenue_schedule(*args, **kwargs)

def distribute_opex_monthly(*args, **kwargs):
    return build_opex_schedule(*args, **kwargs)

def allocate_capex_monthly(*args, **kwargs):
    return build_capex_schedule(*args, **kwargs)

def compute_depreciation_monthly(*args, **kwargs):
    return build_depreciation_schedule(*args, **kwargs)

def create_capex_and_depr_schedules(*args, **kwargs):
    # Some baselines used a combined helper. Try a preexisting impl if present,
    # otherwise emulate via the new API.
    if 'create_capex_and_depreciation_schedules' in globals():
        return globals()['create_capex_and_depreciation_schedules'](*args, **kwargs)
    capex = build_capex_schedule(*args, **kwargs)
...
# ---- notorious one: apply_ramps_and_scenarios ---------------------------------
def apply_ramps_and_scenarios(*args, **kwargs):
    """
    Legacy facade. Attempts to call a newer 'time modifiers' function if present;
    otherwise returns the first arg unchanged (no-op) to remain conservative.
    """
    import logging
    log = logging.getLogger(__name__)
    df = args[0] if args else kwargs.get("df")
    if df is None:
        raise ValueError("apply_ramps_and_scenarios shim expected a DataFrame as first argument")
    for fn_name in ("apply_time_modifiers","apply_ramp_profiles","apply_revenue_ramps","apply_opex_ramps"):
        fn = globals().get(fn_name)
        if callable(fn):
            try:
                return fn(*args, **kwargs)
                ...
                try:
                    ...
                    pass
    log.warning("apply_ramps_and_scenarios shim used; no newer fn found; returning input unchanged.")
    return df
...
try:
    ...
    __all__ = []
for _n in (
    "create_revenue_schedule","create_opex_schedule","create_capex_schedule","create_depreciation_schedule",
    "create_capex_and_depr_schedules","calculate_steady_state_revenue","distribute_revenue_monthly",
    "distribute_opex_monthly","allocate_capex_monthly","compute_depreciation_monthly",
    "apply_ramps_and_scenarios"
):
    if _n not in __all__:
        __all__.append(_n)
