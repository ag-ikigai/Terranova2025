# src/terra_nova/modules/m6_balance_sheet/runner.py
...
def _read_parquet(p: Path) -> pd.DataFrame:
    # Added fallback engine for robustness
    try:
...
def _discover_artifacts(out_dir: Path) -> dict:
    ...
    # Added alternative names for M2 P&L and M3 Debt based on pipeline logs
    cand = {
        "m2_pl": ["m2_pl_schedule.parquet", "m2_profit_and_loss_stub.parquet"],
        "m2_wc": ["m2_working_capital_schedule.parquet"],
        "m3_debt": ["m3_revolver_schedule.parquet", "m3_financing_schedule.parquet"],
...
def _normalize_month_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not mi:
        raise AssertionError(f"[M6] Month_Index not found. Columns: {list(df.columns)[:10]}")
    if mi != "Month_Index":
        df = df.rename(columns={mi: "Month_Index"})
    # Ensure Month_Index is integer and handle potential NaNs if coercion fails
    df["Month_Index"] = pd.to_numeric(df["Month_Index"], errors='coerce')
    if df["Month_Index"].isnull().any():
         raise ValueError("[M6] Month_Index contains non-numeric or NaN values.")
...
def run_m6(out_dir: str, currency: str, inspect_only: bool = False, start_share_capital: float = 0.0) -> None:
    """
    Build M6 balance sheet (beta).
    """
    out = Path(out_dir).resolve()
    out.mkdir(parents=True, exist_ok=True)

    # Updated notes to reflect the current calculation method and canonical naming
    notes = "M6 beta: Cash_and_Cash_Equivalents is currently calculated as a balancing item; true cash wiring expected in v7.5."
    found = _discover_artifacts(out)
    dbg = {"found": found, "currency": currency, "notes": notes}
    ...
    try:
        ...
        print(f"[M6][FAIL] Error during artifact loading or normalization: {e}")
        raise
