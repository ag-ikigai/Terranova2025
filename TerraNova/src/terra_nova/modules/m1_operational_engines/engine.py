...
def create_capex_and_depreciation_schedules(capex_schedule_df: pd.DataFrame, calendar_df: pd.DataFrame):
    ...
    monthly_capex_df["CAPEX_Outflow_NAD_000"] = monthly_capex_df["CAPEX_Outflow_NAD_000"].fillna(0.0)
    ...
    for _, row in capex_schedule_df.iterrows():
        ...
        amount = float(row["Amount_NAD_000"]) if not pd.isna(row["Amount_NAD_000"]) else 0.0
        start_month = int(row["Month"]) + 1
        if life_years and life_years > 0:
            life_months = int(life_years) * 12
            monthly_dep = amount / life_months if life_months > 0 else 0.0
            end_month = min(start_month + life_months - 1, horizon)
            mask = (monthly_depreciation_df["Month_Index"] >= start_month) & (monthly_depreciation_df["Month_Index"] <= end_month)
            monthly_depreciation_df.loc[mask, "Depreciation_NAD_000"] += monthly_dep
...
def create_opex_schedule(opex_detail_df: pd.DataFrame, calendar_df: pd.DataFrame, opex_multiplier: float) -> pd.DataFrame:
    ...
    if not required_cols.issubset(opex_detail_df.columns):
        missing = sorted(required_cols - set(opex_detail_df.columns))
        raise ValueError(f"opex_detail_df missing required columns: {missing}")
...
def calculate_steady_state_revenue(revenue_assumptions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not set(cols).issubset(revenue_assumptions_df.columns):
        missing = sorted(set(cols) - set(revenue_assumptions_df.columns))
        raise ValueError(f"revenue_assumptions_df missing required columns: {missing}")
...
def apply_ramps_and_scenarios(steady_state_revenue_df: pd.DataFrame, rev_ramp_seasonality_df: pd.DataFrame, price_multiplier: float, yield_multiplier: float) -> pd.DataFrame:
    ...
    merged = pd.merge(adj, rfs, on="Crop", how="inner")
    src = set(adj["Crop"]); got = set(merged["Crop"])
    if src != got:
        missing = sorted(src - got)
        raise ValueError(f"Data mismatch in 'apply_ramps_and_scenarios'. Missing ramps for crops: {missing}")
    df = merged
    for i in range(1, 6):
...
def distribute_revenue_monthly(adjusted_annual_revenue_df: pd.DataFrame, rev_ramp_seasonality_df: pd.DataFrame, calendar_df: pd.DataFrame) -> pd.DataFrame:
    ...
    season_long["Month"] = season_long["M"].str.replace("M", "").astype(int)
    cal_expanded = pd.merge(calendar_df, adjusted_annual_revenue_df, on="Year", how="inner")
    src_crops = set(adjusted_annual_revenue_df["Crop"]); season_crops = set(season_long["Crop"])
    if not src_crops.issubset(season_crops):
        missing = sorted(src_crops - season_crops)
        raise ValueError(f"Data mismatch in 'distribute_revenue_monthly'. Missing seasonality for: {missing}")
    expanded = pd.merge(cal_expanded, season_long, on=["Crop", "Month"], how="inner")
